...
class PPK2Main(QMainWindow):
    ...
    def init_guest_parameters(self):
        ...
        self.advertising_interval = 150
        self.connection_interval = 60
        self.running_mode = 0
        self.fast_time = 0
        self.spare_0 = 0
        self.spare_1 = 0
    ...
    def create_right_column(self):
        ...
        default_commands = [
            "AA 03 06 01 EE",
            "AA 02 20 01 04 D0 99 55 85 5D B1 EE",
            "AA 01 04 00 EE"
        ]
    ...
    def _save_downsampled_split(self, base_path, downsampled_samples, excel_limit):
        ...
        try:
            ...
            if hasattr(self, 'wakeup_timestamps') and self.wakeup_timestamps:
                print(f"📋 WAKEUP TIMESTAMPS TO APPLY: {len(self.wakeup_timestamps)} wakeup(s) recorded in this measurement")
            ...
            if current_running_state == 3 and self.current_wakeup_state == 0:
                    ...
                    if self.wakeup_detection_cooldown <= 0:
                        ...
                        if len(self.wakeup_sample_buffer) >= self.WAKEUP_WINDOW_SIZE:
                            ...
                            if window_mean > wakeup_threshold:
                                print(f"🔔 WAKEUP CONFIRMED: Baseline={self.wakeup_baseline:.1f}µA, Window Mean={window_mean:.1f}µA, Threshold={wakeup_threshold:.1f}µA")
                ...
                if (current_running_state != 3 or self.current_wakeup_state != 0):
                    ...
                    if current_running_state in [1, 2, 3] and self.current_wakeup_state != 0:
                        print(f"🔄 WAKEUP RESET: Running_State changed to {current_running_state} (away from wakeup state 0), resetting Wakeup_State to 0")
