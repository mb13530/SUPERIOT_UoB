...
class PPK2_API():
    def __init__(self, port: str, **kwargs):
        ...
        **kwargs - keyword arguments to pass to the pySerial constructor
        ...
        # Use a high baud rate by default to sustain high sample throughput.
        # Allow caller to override via kwargs; otherwise default to 1,000,000 baud.
        try:
            if getattr(self.ser, 'baudrate', None) and 'baudrate' in kwargs:
                # Respect explicit caller-provided baudrate
                pass
            else:
                self.ser.baudrate = 1_000_000
        except Exception:
            # Fallback in case some drivers reject non-standard rates
    ...
    def _write_serial(self, cmd_tuple):
        ...
        try:
            cmd_packed = self._pack_struct(cmd_tuple)
            self.ser.write(cmd_packed)
            logging.debug(f"Sent: {cmd_packed.hex()}")  # <-- new log line
        except Exception as e:
            logging.error(f"An error occurred when writing to serial port: {e}")
    ...
    def _read_metadata(self):
        """Read metadata from the device, tolerating non-UTF8 bytes."""
        for _ in range(5):
            read = self.ser.read(self.ser.in_waiting)
            time.sleep(0.1)
            if read != b'':
                try:
                    ...
                    # Replace or ignore invalid bytes so that we can still check for "END"
                    decoded = read.decode("utf-8", errors="replace")
                if "END" in decoded:
                    return decoded
        return ""  # Return empty string if metadata could not be read
    ...
    def get_data(self):
        ...
        sampling_data = self.ser.read(self.ser.in_waiting)
        # if sampling_data:
        #     logging.debug(f"Received: {sampling_data.hex()}")  # <-- new log line
        return sampling_data
    ...
    def get_samples(self, buf):
        """
        Process the new bytes (buf) together with any leftover bytes from before.
        This method attempts to re-synchronize if a 4-byte sample looks implausible.
        Returns a tuple: (list_of_samples, raw_digital_output)
        """
        sample_size = 4
        samples = []
        raw_digital_output = []

        # Combine previous remainder with new data
        full_buf = self.remainder.get("sequence", b'') + buf
        # Reset the remainder
        self.remainder["sequence"] = b''
        self.remainder["len"] = 0

        idx = 0
        # Process as long as there is at least one full sample left
        while idx + sample_size <= len(full_buf):
            chunk = full_buf[idx:idx + sample_size]
            try:
                adc_val = self._digital_to_analog(chunk)
            ...
            # Check plausibility; adjust the threshold as needed.
            # In this example, if the measurement is negative or unreasonably high,
            # assume misalignment.
            if measurement is not None and (measurement < 0 or measurement > 1e7):
                # Suspect misalignment; shift one byte and try to re-sync
                idx += 1
                continue

            # Accept the sample if plausible.
        ...
        # Save any leftover bytes (less than one sample)
        if idx < len(full_buf):
            self.remainder["sequence"] = full_buf[idx:]
            self.remainder["len"] = len(full_buf) - idx
        else:
            self.remainder["sequence"] = b''
            self.remainder["len"] = 0
